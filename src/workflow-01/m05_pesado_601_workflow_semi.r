...
FEhist_base <- function( pinputexps)
{
  ...
  param_local$meta$script <- "/src/workflow-01/04_551_FE_historia.r"
...
TS_strategy_base9 <- function( pinputexps )
{
  ...
  # Atencion  0.2  de  undersampling de la clase mayoritaria,  los CONTINUA
  # 1.0 significa NO undersampling
  param_local$train$undersampling <- 0.80
  param_local$train$clase_minoritaria <- c( "BAJA+1", "BAJA+2")
...
HT_tuning_base <- function( pinputexps, bypass=FALSE)
{
  ...
  param_local$lgb_param <- list(
    ...
    extra_trees = FALSE,
    # Parte variable
    learning_rate = c( 0.09, 0.2 ),
    feature_fraction = c( 0.1, 0.9 ),
    num_leaves = c( 8L,  256L,  "integer" ),
    min_data_in_leaf = c( 4000L, 12000L, "integer" )
  )
